...
{
    public class DepartamentosControllerUnitTests
    {
        private readonly Mock<ICrudService<DepartamentoDto, CreateDepartamentoDto, UpdateDepartamentoDto>> _mockSvc;
        private readonly DepartamentosController _controller;

        public DepartamentosControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<DepartamentoDto, CreateDepartamentoDto, UpdateDepartamentoDto>>();
            _controller = new DepartamentosController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateDepartamentoDto("Depto");
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateDepartamentoDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateDepartamentoDto("");
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateDepartamentoDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<DepartamentoDto>{ new DepartamentoDto(1, "Depto") };
        ...
        {
            var item = new DepartamentoDto(1, "Depto");
        ...
        {
            var dto = new UpdateDepartamentoDto("Depto");
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateDepartamentoDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
