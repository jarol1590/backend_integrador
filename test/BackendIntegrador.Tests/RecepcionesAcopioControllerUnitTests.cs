...
{
    public class RecepcionesAcopioControllerUnitTests
    {
        private readonly Mock<ICrudService<RecepcionAcopioDto, CreateRecepcionAcopioDto, UpdateRecepcionAcopioDto>> _mockSvc;
        private readonly RecepcionesAcopioController _controller;

        public RecepcionesAcopioControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<RecepcionAcopioDto, CreateRecepcionAcopioDto, UpdateRecepcionAcopioDto>>();
            _controller = new RecepcionesAcopioController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateRecepcionAcopioDto(1,1,DateTime.UtcNow,1,null,10m);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateRecepcionAcopioDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Existe"));
        ...
        {
            var item = new RecepcionAcopioDto(1,1,1,DateTime.UtcNow,1,null,10m);
        ...
        {
            var dto = new UpdateRecepcionAcopioDto(1,1,DateTime.UtcNow,1,null,10m);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateRecepcionAcopioDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
