...
{
    public class FincasControllerUnitTests
    {
        private readonly Mock<ICrudService<FincaDto, CreateFincaDto, UpdateFincaDto>> _mockSvc;
        private readonly FincasController _controller;

        public FincasControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<FincaDto, CreateFincaDto, UpdateFincaDto>>();
            _controller = new FincasController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateFincaDto("Finca", null, null, null, 1, 1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateFincaDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateFincaDto("", null, null, null, 0, 0);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateFincaDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<FincaDto>{ new FincaDto(1,"Finca",null,null,null,1,1) };
        ...
        {
            var item = new FincaDto(1,"Finca",null,null,null,1,1);
        ...
        {
            var dto = new UpdateFincaDto("Finca", null, null, null, 1, 1);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateFincaDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
