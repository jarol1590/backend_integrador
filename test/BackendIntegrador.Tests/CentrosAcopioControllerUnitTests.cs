...
{
    public class CentrosAcopioControllerUnitTests
    {
        private readonly Mock<ICrudService<CentroAcopioDto, CreateCentroAcopioDto, UpdateCentroAcopioDto>> _mockSvc;
        private readonly CentrosAcopioController _controller;
        ...
        {
            var dto = new CreateCentroAcopioDto("Centro", null, null, null, 1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateCentroAcopioDto>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new InvalidOperationException("Existe"));

            var result = await _controller.Create(dto, CancellationToken.None);
            if (result.Result is BadRequestObjectResult) (result.Result as BadRequestObjectResult)!.Should().NotBeNull();
            else { var obj = result.Result as ObjectResult; obj.Should().NotBeNull(); obj!.StatusCode.Should().Be(409); }
        }

        [Fact]
        public async Task Create_InvalidData_ReturnsBadRequest()
        {
            var dto = new CreateCentroAcopioDto("", null, null, null, 1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateCentroAcopioDto>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new InvalidOperationException("Datos inválidos"));

            var result = await _controller.Create(dto, CancellationToken.None);
            if (result.Result is BadRequestObjectResult) (result.Result as BadRequestObjectResult)!.Should().NotBeNull();
            else { var obj = result.Result as ObjectResult; obj.Should().NotBeNull(); obj!.StatusCode.Should().Be(400); }
        }

        [Fact]
        public async Task GetAll_ReturnsOk()
        {
            var list = new List<CentroAcopioDto>{ new CentroAcopioDto(1,"Centro",null,null,null,1) };
        ...
        {
            var item = new CentroAcopioDto(1,"Centro",null,null,null,1);
        ...
        {
            var dto = new UpdateCentroAcopioDto("Centro", null, null, null, 1);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateCentroAcopioDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        ...
        public async Task Delete_AcceptsNoContentOrOkWithStatus204()
        {
            ...
            else if (action is OkObjectResult ok) { var statusProp = ok.Value.GetType().GetProperty("status"); statusProp.Should().NotBeNull(); statusProp!.GetValue(ok.Value).Should().Be(204); }
