...
{
    public class ProductoresControllerUnitTests
    {
        private readonly Mock<ICrudService<ProductorDto, CreateProductorDto, UpdateProductorDto>> _mockSvc;
        private readonly ProductoresController _controller;

        public ProductoresControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<ProductorDto, CreateProductorDto, UpdateProductorDto>>();
            _controller = new ProductoresController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateProductorDto("Prod","123",null,1,1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateProductorDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateProductorDto("","",null,0,0);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateProductorDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<ProductorDto>{ new ProductorDto(1, "Prod", "123", null, 1, 1) };
        ...
        {
            var item = new ProductorDto(1, "Prod", "123", null, 1, 1);
        ...
        {
            var dto = new UpdateProductorDto("Prod","123",null,1,1);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateProductorDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
