...
{
    public class LotesControllerUnitTests
    {
        private readonly Mock<ICrudService<LoteDto, CreateLoteDto, UpdateLoteDto>> _mockSvc;
        private readonly LotesController _controller;

        public LotesControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<LoteDto, CreateLoteDto, UpdateLoteDto>>();
            _controller = new LotesController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateLoteDto(1,1,10m,null);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateLoteDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateLoteDto(0,0,0m,null);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateLoteDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<LoteDto>{ new LoteDto(1,1,1,10m,null) };
        ...
        {
            var item = new LoteDto(1,1,1,10m,null);
        ...
        {
            var dto = new UpdateLoteDto(1,1,10m,null);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateLoteDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
