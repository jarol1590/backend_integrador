...
{
    public class OrdenosControllerUnitTests
    {
        private readonly Mock<ICrudService<OrdenoDto, CreateOrdenoDto, UpdateOrdenoDto>> _mockSvc;
        private readonly OrdenosController _controller;

        public OrdenosControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<OrdenoDto, CreateOrdenoDto, UpdateOrdenoDto>>();
            _controller = new OrdenosController(_mockSvc.Object, null!);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateOrdenoDto(DateTime.UtcNow, null, 5m, 1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateOrdenoDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateOrdenoDto(DateTime.UtcNow, null, 0m, 0);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateOrdenoDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<OrdenoDto>{ new OrdenoDto(1, null, DateTime.UtcNow, null, 5m, 1) };
        ...
        {
            var item = new OrdenoDto(1, null, DateTime.UtcNow, null, 5m, 1);
        ...
        {
            var dto = new UpdateOrdenoDto(DateTime.UtcNow, null, 5m, 1);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateOrdenoDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
