...
namespace BackendIntegrador.Tests
{
    public class TransportesControllerUnitTests
    {
        ...
        {
            var dto = new CreateTransporteDto("ABC123", DateTime.UtcNow, null, null);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateTransporteDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateTransporteDto("", DateTime.UtcNow, null, null);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateTransporteDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<TransporteDto>{ new TransporteDto(1, "ABC123", DateTime.UtcNow, null, null) };
        ...
        {
            var item = new TransporteDto(1, "ABC123", DateTime.UtcNow, null, null);
        ...
        {
            var dto = new UpdateTransporteDto("ABC123", DateTime.UtcNow, null, null);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateTransporteDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
