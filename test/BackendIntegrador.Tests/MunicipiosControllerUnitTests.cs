...
{
    public class MunicipiosControllerUnitTests
    {
        private readonly Mock<ICrudService<MunicipioDto, CreateMunicipioDto, UpdateMunicipioDto>> _mockSvc;
        private readonly MunicipiosController _controller;

        public MunicipiosControllerUnitTests()
        {
            _mockSvc = new Mock<ICrudService<MunicipioDto, CreateMunicipioDto, UpdateMunicipioDto>>();
            _controller = new MunicipiosController(_mockSvc.Object);
        }

        [Fact]
        public async Task Create_ConflictOrBadRequest()
        {
            var dto = new CreateMunicipioDto("Mun", 1);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateMunicipioDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Existe"));
        ...
        {
            var dto = new CreateMunicipioDto("", 0);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateMunicipioDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new System.InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
        ...
        {
            var list = new List<MunicipioDto>{ new MunicipioDto(1, "Mun", 1) };
        ...
        {
            var item = new MunicipioDto(1, "Mun", 1);
        ...
        {
            var dto = new UpdateMunicipioDto("Mun", 1);
            _mockSvc.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateMunicipioDto>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
