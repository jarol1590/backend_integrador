...
namespace BackendIntegrador.Tests
{
    public class MuestrasControllerUnitTests
    {
        ...
        public MuestrasControllerUnitTests()
        {
            ...
            _controller = new MuestrasController(_mockSvc.Object, null!);
        }
        ...
        {
            var dto = new CreateMuestraDto(0,0,DateTime.UtcNow);
            _mockSvc.Setup(s => s.CreateAsync(It.IsAny<CreateMuestraDto>(), It.IsAny<CancellationToken>())).ThrowsAsync(new InvalidOperationException("Datos inválidos"));
            var result = await _controller.Create(dto, CancellationToken.None);
